...
public class user_manage extends HttpServlet {
    ...
    public void delete_things(HttpServletRequest request,HttpServletResponse response) throws UnsupportedEncodingException{
        ...
        db conn=new db();
        String sql="DELETE FROM user WHERE user_name='"+ delete_name + "'";
    ...
    public void modify_things(HttpServletRequest request,HttpServletResponse response) throws UnsupportedEncodingException{
        ...
        db conn=new db();
        String sql="UPDATE user SET money='"+money+"'WHERE user_name='"+name+"'";
        System.out.println(sql);
        conn.executeUpdate(sql);
        conn.close();
        System.out.println("操作数据完毕，关闭了数据库！");
    ...
    public void add_user(HttpServletRequest request,HttpServletResponse response) throws IOException {
        ...
        String low=request.getParameter("password");
        String high=request.getParameter("email");
        String wind_direction=request.getParameter("money");
        ...
        db conn=new db();
        String sql="INSERT user (user_name,password,email,money,create_date) values( '"+city+"','"+low+"','"+high+"','"+wind_direction+"','"+date+"')";
        System.out.println(sql);
        conn.executeUpdate(sql);
        conn.close();
        System.out.println("操作数据完毕，关闭了数据库！");
