...
var time_arr = function(type) { 
	...
	//大概打算获取开始时间，当前时间的四小时前开始绘制，因此一定能画出来
	if(type.indexOf('wh')!=-1){//生成外汇时间段
		var timeArr = new Array();
			timeArr.push('00:01');
			timeArr.concat(getNextTime('00:01', '00:40', 1, timeArr)); 
		return timeArr;
	}
...
var get_m_data = function(m_data,type) {
	...
	// getNextTime(times[times.length - 1], '16:00', 1, times)
	console.log(times)
...
function initCMOption(m_data,type){
	var m_datas = get_m_data(m_data,type); 
	return {
		tooltip: { //弹框指示器
			trigger: 'axis',
